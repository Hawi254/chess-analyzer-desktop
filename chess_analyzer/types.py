...
class ClassificationResult:
    ...
    tactic_type: Optional[TacticType]
...
class MoveAnalysisContext:
    ...
    move_characteristics: MoveCharacteristics
...
class AnnotationContext:
    classification: Optional[MoveClassification]
    centipawn_loss: Optional[int]
    engine_lines: List[FormattedEngineLine]
    user_comment: str
...
class ProcessedGameResult:
    annotated_game: Optional["chess.pgn.Game"]
...
@dataclass
class RunReport:
    results: List[ProcessedGameResult]
    processed_game_count: int
    user_found_in_games: bool
...
class ParsedGame:
    metadata: GameMetadata
    slices: List[GameSlice]
    unique_fens: List[FEN]
...
class PositionStats:
    ...
    average_cpl: float
    brilliant_count: int
    great_move_count: int
    good_move_count: int
    dubious_move_count: int
    ...
    move_count_in_time_trouble: int
    eval_std_dev: Optional[float]

@dataclass
class GameContext:
    game_id: str
    raw_game: "chess.pgn.Game"
    settings: "AnalysisSettings"
    run_config: "RunConfig"
    persistence_service: "PersistenceService"
    engine_service: Optional["EngineService"] = None
    parsed_game: Optional[ParsedGame] = None
...
class NarrativeContext:
    std_dev_white: float
    std_dev_black: float
    combined_blunders: int
    white_acpl: Optional[float]
    black_acpl: Optional[float]
    white_blunders: int
    black_blunders: int
